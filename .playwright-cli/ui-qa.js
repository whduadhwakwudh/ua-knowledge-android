// src/connection-store.js
var TOKEN_PATTERN = /^uak_[A-Za-z0-9_-]{43}$/;
var HTTP_LOCAL_HOSTS = /* @__PURE__ */ new Set(["localhost", "127.0.0.1"]);
...
function validateLlmApiKey(value) {
...
var ALLOWED_ATTRS = ["href", "title", "alt", "src", "class", "data-wiki-target"];
var WIKI_LINK_RE = /\[\[([^\]|]+)(?:\|([^\]]+))?\]\]/g;
...
function renderMarkdown(markdown) {
  const withWikiLinks = String(markdown ?? "").replace(WIKI_LINK_RE, (_m, target, label) => {
    ...
    return '<a href="#wiki:' + encodeURIComponent(safeTarget) + '" class="wiki-link" data-wiki-target="' + escHtmlAttr(safeTarget) + '">' + escHtmlAttr(safeLabel) + "</a>";
  });
  const raw = f.parse(withWikiLinks, { gfm: true, breaks: false }) ?? "";
...
function folderBreadcrumbs(folderPath = "") {
...
// src/ui.js
var HIDDEN = "hidden";
var STAR_PATH = "M12 3.6l2.56 5.2 5.73.83-4.14 4.04.98 5.7L12 16.72l-5.13 2.7.98-5.7L3.7 9.63l5.73-.83L12 3.6z";
var THEME_LABEL = { light: "\u6D45\u8272", dark: "\u6DF1\u8272", system: "\u8DDF\u968F\u7CFB\u7EDF" };
function esc(value) {
  return String(value).replace(/[&<>"']/g, (c) => ({ "&": "&amp;", "<": "&lt;", ">": "&gt;", '"': "&quot;", "'": "&#39;" })[c]);
}
...
function todayGreeting() {
  try {
    return new Intl.DateTimeFormat("zh-CN", { month: "numeric", day: "numeric", weekday: "short" }).format(/* @__PURE__ */ new Date());
  } catch {
    return "";
  }
}
function clampReadScale(value, min = 0.7, max = 2.5) {
...
function mountApp(container, wiring = {}) {
  const doc = container.ownerDocument;
  const $2 = (selector) => container.querySelector(selector);
  ...
  const expandedSidebarFolders = /* @__PURE__ */ new Set();
  let lastSidebarCategory = null;
  ...
  function openConnectionSheet() {
    ...
    if ($2("#conn-base-url")) {
      $2("#conn-base-url").value = state.baseUrl ?? "";
      $2("#conn-token").value = "";
      $2("#conn-llm-key").value = "";
    }
    if ($2("#conn-storage-warning")) {
      $2("#conn-storage-warning").textContent = state.storageWarning ?? "";
      toggleHidden($2("#conn-storage-warning"), !state.storageWarning);
    }
    setConnectionStatus("");
  }
  function setConnectionBusy(busy) {
    const testBtn = $2("#conn-test");
    const saveBtn = $2("#conn-save");
    if (testBtn) {
  ...
  function setConnectionStatus(text2, _kind) {
    const el = $2("#conn-status");
    if (el) el.textContent = text2 ?? "";
  }
  function showConnectionErrors(errors = {}) {
    const fields = { baseUrl: $2("#conn-base-url"), token: $2("#conn-token"), llmKey: $2("#conn-llm-key") };
    const errEls = { baseUrl: $2("#conn-base-url-err"), token: $2("#conn-token-err"), llmKey: $2("#conn-llm-key-err") };
  ...
  function submitConnection(action) {
    const baseUrl = ($2("#conn-base-url")?.value ?? "").trim();
    const token = $2("#conn-token")?.value ?? "";
    const llmApiKey = $2("#conn-llm-key")?.value ?? "";
    const errors = {};
    if (!baseUrl) {
  ...
  function applyTheme(value) {
    ...
    if ($2("#set-appearance-sub")) $2("#set-appearance-sub").textContent = THEME_LABEL[value] ?? value;
  ...
  function noteCardHtml(d) {
    return '<article class="note-card" data-open="' + esc(d.id) + '" role="button" tabindex="0" aria-label="\u6253\u5F00\u7B14\u8BB0\uFF1A' + esc(d.title) + '"><svg class="note-ic" viewBox="0 0 24 24" aria-hidden="true"><path d="M6 3.5h8l4 4v13H6z"/><path d="M14 3.5v4h4"/></svg><div class="note-body"><h3 class="note-title">' + esc(d.title) + '</h3><p class="note-excerpt">' + esc(d.excerpt ?? "") + '</p></div><div class="note-right"><span class="note-label">' + esc(d.label ?? "") + '</span><span class="note-time">' + esc(d.updated ?? "") + "</span></div></article>";
  }
  function favRowHtml(d) {
    const isFav = state.favorites.includes(d.id);
    const on2 = isFav ? " on" : "";
    return '<div class="list-row"><div class="body" data-open="' + esc(d.id) + '" role="button" tabindex="0" aria-label="\u6253\u5F00\u7B14\u8BB0\uFF1A' + esc(d.title) + '"><div class="title">' + esc(d.title) + '</div><div class="sub">' + esc(d.excerpt ?? "") + '</div></div><button class="star-btn' + on2 + '" data-toggle-fav="' + esc(d.id) + '" aria-pressed="' + (isFav ? "true" : "false") + '" aria-label="' + (isFav ? "\u53D6\u6D88\u6536\u85CF" : "\u6536\u85CF") + "\uFF1A" + esc(d.title) + '"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="' + STAR_PATH + '"/></svg></button></div>';
  }
  function artifactRowHtml(a) {
    const meta = [formatSize(a.size), "\u6821\u9A8C " + String(a.sha256 ?? "").slice(0, 8)].filter(Boolean).join(" \xB7 ");
    return '<div class="list-row"><div class="body"><div class="title">' + esc(a.title) + '</div><div class="sub">' + esc(meta) + '</div></div><button class="btn-primary" data-download-id="' + esc(a.id) + '" style="min-height:36px;padding:8px 12px;font-size:13px;border-radius:10px;">\u4E0B\u8F7D</button></div>';
  }
  function renderSidebar(state2) {
    const allDocs = state2.allDocuments ?? state2.documents ?? [];
    const current = state2.category ?? "all";
    const nav = $2("#sidebar-cats");
    if (nav) {
      if (current !== lastSidebarCategory) {
        const segments = current === "all" ? [] : current.split("/").filter(Boolean);
        let ancestor = "";
        for (const segment of segments.slice(0, -1)) {
          ancestor = ancestor ? `${ancestor}/${segment}` : segment;
          expandedSidebarFolders.add(ancestor);
        }
        lastSidebarCategory = current;
      }
      ...
      if (state2.folderTree) appendFolders(state2.folderTree.folders, 1);
      else {
        for (const category of state2.categories ?? []) {
          items.push({
            value: category,
            label: category,
      ...
      nav.innerHTML = items.map((item) => {
        ...
        const disclosure = item.hasChildren ? '<button class="sidebar-disclosure" data-folder-toggle="' + esc(item.value) + '" aria-expanded="' + (expanded ? "true" : "false") + '" aria-label="' + (expanded ? "\u6536\u8D77" : "\u5C55\u5F00") + esc(item.label) + '"><svg viewBox="0 0 20 20" aria-hidden="true"><path d="m7 4 6 6-6 6"/></svg></button>' : '<span class="sidebar-disclosure-placeholder" aria-hidden="true"></span>';
        return '<div class="sidebar-tree-row" data-depth="' + item.depth + '" style="--tree-indent:' + Math.min(item.depth, 5) * 12 + 'px">' + disclosure + '<button class="sidebar-cat' + (current === item.value ? " active" : "") + '" data-category="' + esc(item.value) + '" aria-pressed="' + (current === item.value ? "true" : "false") + '"><svg class="sidebar-folder-icon" viewBox="0 0 24 24" aria-hidden="true"><path d="M3 7a2 2 0 0 1 2-2h4l2 2h8a2 2 0 0 1 2 2v8a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z"/></svg><span class="cat-name">' + esc(item.label) + '</span><span class="cat-count">' + item.count + "</span></button></div>";
      }).join("");
    }
    if ($2("#sidebar-conn")) {
      $2("#sidebar-conn").textContent = state2.connection === "unconfigured" ? "\u672A\u8FDE\u63A5" : state2.baseUrl ?? "\u5DF2\u8FDE\u63A5";
    }
  }
  function renderFolderBrowser(state2, searching) {
    const wrap = $2("#folder-browser");
    if (!wrap) return;
    const tree = state2.folderTree;
    const folderPath = state2.category === "all" ? "" : state2.category ?? "";
    const folder = findFolder(tree, folderPath);
    toggleHidden(wrap, searching || !folder);
    if (!folder) return;
    const breadcrumbs = folderBreadcrumbs(folderPath);
    const crumbs = $2("#folder-breadcrumbs");
    if (crumbs) {
      crumbs.innerHTML = breadcrumbs.map(
        (item, index) => '<button class="folder-crumb" data-category="' + esc(item.path === "" ? "all" : item.path) + '"' + (index === breadcrumbs.length - 1 ? ' aria-current="page"' : "") + ">" + esc(item.label) + "</button>"
      ).join('<span class="folder-sep">\u203A</span>');
    }
    const list = $2("#folder-list");
    if (list) {
      list.innerHTML = (folder.folders ?? []).map(
        (child) => '<button class="folder-card" data-category="' + esc(child.path) + '"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M3 7a2 2 0 0 1 2-2h4l2 2h8a2 2 0 0 1 2 2v8a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z"/></svg><span class="folder-card-name">' + esc(child.name) + '</span><span class="folder-card-count">' + child.count + '</span><svg class="folder-card-chevron" viewBox="0 0 20 20" aria-hidden="true"><path d="m7 4 6 6-6 6"/></svg></button>'
      ).join("");
    }
  }
  function openSidebar() {
    const layer = $2("#sidebar-layer");
    if (!layer) return;
    layer.classList.add("open");
    layer.setAttribute("aria-hidden", "false");
  }
  ...
  function renderCategoryChips(state2) {
    const categories = state2.categories ?? [];
    const searching = (state2.query ?? "").trim() !== "";
    const chipRow = $2("#category-chips");
    if (!chipRow) return;
    ...
    chipRow.innerHTML = items.map(
      (item) => '<button class="chip' + (current === item.value ? " chip-on" : "") + '" data-category="' + esc(item.value) + '" aria-pressed="' + (current === item.value ? "true" : "false") + '">' + esc(item.label) + "</button>"
    ).join("");
    chipRow.classList.remove(HIDDEN);
  }
  function renderHome(state2) {
    const query = (state2.query ?? "").trim();
    const documents = state2.documents ?? [];
    const noDocs = documents.length === 0;
    const searching = query !== "";
    if ($2("#home-greeting")) {
      const g = todayGreeting();
      if (g) $2("#home-greeting").textContent = g + " \xB7 \u79BB\u7EBF\u53EF\u7528";
    }
    const showPrompt = state2.connection === "unconfigured" && noDocs;
    toggleHidden($2("#connect-prompt-wrap"), !showPrompt);
    toggleHidden($2("#sync-card"), showPrompt);
    toggleHidden($2("#auth-error-wrap"), state2.connection !== "auth-error");
    renderCategoryChips(state2);
    renderFolderBrowser(state2, searching);
    toggleHidden($2("#btn-shuffle"), searching);
    if ($2("#recent-count")) $2("#recent-count").textContent = documents.length + " \u7BC7";
    toggleHidden($2("#search-head"), !searching);
    if ($2("#sh-count")) $2("#sh-count").textContent = documents.length + " \u7BC7";
    if ($2("#recent-list")) {
      $2("#recent-list").innerHTML = documents.map(noteCardHtml).join("");
    }
    if (searching) {
      toggleHidden($2("#search-empty"), documents.length > 0);
      if ($2("#search-empty-q")) $2("#search-empty-q").textContent = query;
      toggleHidden($2("#recent-empty"), true);
    } else {
      toggleHidden($2("#search-empty"), true);
      toggleHidden($2("#recent-empty"), !(noDocs && state2.connection !== "unconfigured"));
    }
  }
  function renderSyncCard(state2) {
    const statusEl = $2("#sync-status-text");
    const countEl = $2("#sync-count-text");
    const syncBtn = $2("#btn-sync");
    const progress = $2("#sync-progress");
    const fill = $2("#sync-progress-fill");
    if (!statusEl) return;
    let status = "";
    let count = "";
    if (state2.syncing) {
      const counts = state2.syncCounts ?? {};
      if (progress) progress.classList.remove(HIDDEN);
      if (fill) {
        fill.classList.remove("indeterminate");
        fill.style.width = "";
        const total = counts.total ?? 0;
        const done = state2.syncPhase === "download" ? counts.downloaded : state2.syncPhase === "verify" ? counts.verified : void 0;
      ...
      switch (state2.syncPhase) {
        case "manifest":
          status = "\u6B63\u5728\u540C\u6B65 \xB7 \u83B7\u53D6\u76EE\u5F55";
          count = "\u5171 " + (counts.total ?? 0) + " \u4E2A\u6761\u76EE";
          break;
        case "download":
          status = "\u6B63\u5728\u540C\u6B65 \xB7 \u4E0B\u8F7D\u6587\u6863";
          count = (counts.downloaded ?? 0) + " \u7BC7";
          break;
        case "verify":
          status = "\u6B63\u5728\u540C\u6B65 \xB7 \u6821\u9A8C\u5185\u5BB9";
          count = (counts.verified ?? 0) + " \u7BC7";
          break;
        case "commit":
          status = "\u6B63\u5728\u540C\u6B65 \xB7 \u5199\u5165\u672C\u5730\u7F13\u5B58";
          break;
        default:
          status = "\u6B63\u5728\u540C\u6B65 \xB7 \u51C6\u5907\u4E2D";
      }
      ...
      if (state2.syncError) {
        status = "\u540C\u6B65\u5931\u8D25 \xB7 " + state2.syncError;
        count = "\u663E\u793A\u672C\u5730\u7F13\u5B58";
      } else if (state2.syncPhase === "complete" && state2.syncCounts) {
        const { added = 0, updated = 0, removed = 0, unchanged = 0 } = state2.syncCounts;
        if (added === 0 && updated === 0 && removed === 0) {
          status = "\u5DF2\u662F\u6700\u65B0";
        } else {
          status = "\u540C\u6B65\u5B8C\u6210";
          count = `\u65B0\u589E ${added} \xB7 \u66F4\u65B0 ${updated} \xB7 \u79FB\u9664 ${removed} \xB7 \u672A\u53D8 ${unchanged}`;
        }
      } else if (state2.lastSyncAt) {
        status = "\u5DF2\u540C\u6B65 \xB7 " + formatTime(state2.lastSyncAt);
      } else {
        status = "\u5C1A\u672A\u540C\u6B65";
        count = "\u8FDE\u63A5\u8BBE\u5907\u540E\u5373\u53EF\u62C9\u53D6\u5185\u5BB9";
      }
    ...
    if (syncBtn) syncBtn.disabled = !!state2.syncing;
  }
  function renderArtifacts(state2) {
    const all = state2.artifacts ?? [];
    const list = all.length === 0 ? [] : [all.reduce((a, b2) => a.mtime >= b2.mtime ? a : b2)];
    toggleHidden($2("#artifacts-section"), list.length === 0);
    if ($2("#artifacts-count")) $2("#artifacts-count").textContent = list.length + " \u4E2A";
    if ($2("#artifacts-list")) $2("#artifacts-list").innerHTML = list.map(artifactRowHtml).join("");
    toggleHidden($2("#artifacts-empty"), list.length !== 0);
  }
  function renderFavs(state2) {
    const favorites = state2.favorites ?? [];
    const allDocs = state2.allDocuments ?? state2.documents ?? [];
    const favDocs = allDocs.filter((d) => favorites.includes(d.id));
    if ($2("#fav-count")) $2("#fav-count").textContent = favDocs.length + " \u7BC7";
    const groups = $2("#fav-groups");
    if (groups) groups.innerHTML = favDocs.length ? '<div class="grouped fav-group">' + favDocs.map(favRowHtml).join("") + "</div>" : "";
    toggleHidden($2("#fav-groups"), favDocs.length === 0);
    toggleHidden($2("#fav-empty-wrap"), favDocs.length !== 0);
    toggleHidden($2("#fav-empty"), favDocs.length !== 0);
  }
  function renderMe(state2) {
    const documents = state2.allDocuments ?? state2.documents ?? [];
    const dirs = /* @__PURE__ */ new Set();
    for (const d of documents) {
      const slash = d.relativePath ? d.relativePath.indexOf("/") : -1;
      dirs.add(slash > 0 ? d.relativePath.slice(0, slash) : d.relativePath ?? "");
    }
    if ($2("#stat-notes")) $2("#stat-notes").textContent = documents.length;
    if ($2("#stat-sources")) $2("#stat-sources").textContent = dirs.size;
    if ($2("#stat-favs")) $2("#stat-favs").textContent = (state2.favorites ?? []).length;
    if ($2("#set-sync-sub")) {
      $2("#set-sync-sub").textContent = state2.lastSyncAt ? "\u5DF2\u540C\u6B65 \xB7 " + formatTime(state2.lastSyncAt) : "\u672A\u540C\u6B65";
    }
    if ($2("#set-connection-sub")) {
      $2("#set-connection-sub").textContent = state2.connection === "unconfigured" ? "\u672A\u8FDE\u63A5" : state2.baseUrl ?? "\u5DF2\u8FDE\u63A5";
    }
  }
  function renderDetail(state2) {
    const detail = state2.detail;
    if (!detail) {
      screen.classList.remove("detail-open");
      if ($2("#view-detail")) $2("#view-detail").setAttribute("aria-hidden", "true");
      toggleHidden($2("#detail-backlinks"), true);
      toggleHidden($2("#detail-pane"), true);
      currentDetailId = null;
      return;
    }
    currentDetailId = detail.id;
    if ($2("#detail-title")) $2("#detail-title").textContent = detail.title;
    if ($2("#detail-chips")) {
      $2("#detail-chips").innerHTML = (detail.chips ?? []).map((chip) => '<span class="chip">' + esc(chip) + "</span>").join("");
    }
    if ($2("#detail-body")) $2("#detail-body").innerHTML = detail.html ?? "";
    const backlinks = detail.backlinks ?? [];
    const bl = $2("#detail-backlinks");
    if (bl) {
      if (backlinks.length > 0) {
        bl.innerHTML = '<div class="section-head row-between" style="margin-bottom: 8px;"><h2>\u53CD\u5411\u94FE\u63A5</h2><span class="meta num">' + backlinks.length + ' \u7BC7</span></div><div class="grouped">' + backlinks.map(
          (b2) => '<div class="list-row"><div class="body" data-open="' + esc(b2.id) + '" role="button" tabindex="0" aria-label="\u6253\u5F00\u7B14\u8BB0\uFF1A' + esc(b2.title) + '"><div class="title">' + esc(b2.title) + '</div><div class="sub">\u5F15\u7528\u4E86\u672C\u7BC7\u7B14\u8BB0</div></div></div>'
        ).join("") + "</div>";
    ...
    const pane = $2("#detail-pane");
    if (pane) {
      const siblings = detail.siblings ?? [];
      if (siblings.length > 1) {
        pane.innerHTML = '<div class="pane-head">' + esc(detail.label ?? "") + '</div><div class="pane-list">' + siblings.map(
          (s) => '<div class="pane-item' + (s.id === detail.id ? " active" : "") + '" data-open="' + esc(s.id) + '" role="button" tabindex="0" aria-label="\u6253\u5F00\u7B14\u8BB0\uFF1A' + esc(s.title) + '">' + esc(s.title) + "</div>"
        ).join("") + "</div>";
    ...
    const star = $2("#detail-star");
    if (star) {
      const on2 = (state2.favorites ?? []).includes(detail.id);
      star.classList.toggle("on", on2);
      star.setAttribute("aria-pressed", String(on2));
      star.setAttribute("aria-label", on2 ? "\u53D6\u6D88\u6536\u85CF" : "\u6536\u85CF");
    }
    screen.classList.add("detail-open");
    if ($2("#view-detail")) $2("#view-detail").setAttribute("aria-hidden", "false");
    if ($2("#detail-scroll")) $2("#detail-scroll").scrollTop = 0;
  }
  function applyTab(name) {
    $$(".view").forEach((v2) => {
      const active = v2.id === "view-" + name;
      v2.classList.toggle("active", active);
      v2.setAttribute("aria-hidden", active ? "false" : "true");
    });
    $$(".tab").forEach((t) => {
  ...
  function renderAssistant(state2) {
    const messages = state2.assistantMessages ?? [];
    const asking = !!state2.assistantAsking;
    const list = $2("#chat-list");
    const input = $2("#chat-input");
    const sendBtn = $2("#chat-send");
    const emptyWrap = $2("#chat-empty-wrap");
    toggleHidden(emptyWrap, messages.length > 0 || asking);
    if (list) {
      const traceHtml = (trace) => '<details class="assistant-trace"><summary>\u601D\u8003\u8FC7\u7A0B</summary><ol>' + trace.map(
        (step) => '<li data-status="' + esc(step.status ?? "done") + '">' + esc(step.label ?? "") + "</li>"
      ).join("") + "</ol></details>";
      const items = messages.map((m2) => {
        if (m2.role === "user") {
          return '<div class="chat-msg user">' + esc(m2.text) + "</div>";
        }
        return '<div class="chat-msg assistant detail-body">' + (Array.isArray(m2.trace) && m2.trace.length > 0 ? traceHtml(m2.trace) : "") + renderMarkdown(m2.text ?? "") + "</div>";
      });
      if (asking) {
        const trace = state2.assistantTrace ?? [];
        if (trace.length > 0) {
    ...
    if (sendBtn) {
      ...
      sendBtn.innerHTML = asking ? '<svg viewBox="0 0 24 24"><rect x="7" y="7" width="10" height="10" rx="1.5"/></svg>' : '<svg viewBox="0 0 24 24"><path d="M4 12 20 4l-4.5 16-3.5-6.5L4 12z"/></svg>';
    }
  }
  function renderFiles(state2) {
    const files = state2.filesList ?? [];
    const downloadingId = state2.fileDownloadingId ?? null;
    const list = $2("#files-list");
    const empty = $2("#files-empty");
    toggleHidden(empty, files.length > 0);
    if (list) {
      list.innerHTML = files.map((f2) => {
        const downloading = downloadingId === f2.id;
        return '<div class="list-row"><div class="body"><div class="title">' + esc(f2.name) + '</div><div class="sub">' + formatSize(f2.size) + " \xB7 " + formatTime(f2.mtime) + '</div></div><button class="btn-primary" data-file-id="' + esc(f2.id) + '" style="min-height:36px;padding:8px 12px;font-size:13px;border-radius:10px;"' + (downloading ? " disabled" : "") + ">" + (downloading ? "\u4E0B\u8F7D\u4E2D\u2026" : "\u4E0B\u8F7D") + "</button></div>";
      }).join("");
    }
  }
  ...
  const detailScroll = $2("#detail-scroll");
  if (detailScroll) {
    let savedScale = 1;
    try {
  ...
  const searchInput = $2("#search-input");
  if (searchInput) {
    searchInput.addEventListener("input", (e) => {
      const value = e.target.value;
      toggleHidden($2("#search-clear"), value === "");
      wiring.onQueryChange?.(value);
    });
  }
  const clearSearch = () => {
    if (searchInput) searchInput.value = "";
    toggleHidden($2("#search-clear"), true);
    wiring.onQueryChange?.("");
  };
  $2("#search-clear")?.addEventListener("click", clearSearch);
  $2("#search-clear2")?.addEventListener("click", clearSearch);
  $$(".tab").forEach((t) => t.addEventListener("click", () => goTab(t.dataset.tab)));
  $2("#btn-to-profile")?.addEventListener("click", () => goTab("me"));
  $2("#btn-go-home")?.addEventListener("click", () => goTab("home"));
  $2("#btn-sync")?.addEventListener("click", () => wiring.onSync?.());
  $2("#btn-recent-sync")?.addEventListener("click", () => wiring.onSync?.());
  $2("#btn-auth-retry")?.addEventListener("click", () => wiring.onSync?.());
  $2("#btn-shuffle")?.addEventListener("click", () => wiring.onShuffle?.());
  $2("#btn-menu")?.addEventListener("click", openSidebar);
  $2("#sidebar-scrim")?.addEventListener("click", closeSidebar);
  $2("#sidebar-sync")?.addEventListener("click", () => {
    closeSidebar();
    wiring.onSync?.();
  });
  $2("#sidebar-conn-open")?.addEventListener("click", () => {
    closeSidebar();
    openConnectionSheet();
  ...
  $2("#chat-send")?.addEventListener("click", submitChat);
  $2("#chat-input")?.addEventListener("keydown", (e) => {
    if (e.key === "Enter" && !e.shiftKey) {
      e.preventDefault();
      submitChat();
    }
  });
  $2("#chat-input")?.addEventListener("input", (e) => {
    const el = e.target;
    el.style.height = "auto";
    el.style.height = Math.min(el.scrollHeight, 120) + "px";
  });
  $2("#btn-clear-chat")?.addEventListener("click", () => wiring.onClearChat?.());
  $2("#btn-open-connection")?.addEventListener("click", () => {
    openConnectionSheet();
    wiring.onConnectionChange?.({ action: "open" });
  });
  $2("#btn-conn-settings")?.addEventListener("click", () => {
    openConnectionSheet();
    wiring.onConnectionChange?.({ action: "open" });
  });
  $2("#setting-connection")?.addEventListener("click", () => {
    openConnectionSheet();
    wiring.onConnectionChange?.({ action: "open" });
  });
  $2("#setting-appearance")?.addEventListener("click", () => openSheet("sheet-appearance"));
  $2("#setting-files")?.addEventListener("click", () => {
  ...
  $2("#setting-about")?.addEventListener("click", () => openSheet("sheet-about"));
  $2("#conn-test")?.addEventListener("click", () => submitConnection("test"));
  $2("#conn-save")?.addEventListener("click", () => submitConnection("save"));
  $2("#conn-clear")?.addEventListener("click", () => wiring.onConnectionChange?.({ action: "clear" }));
  $2("#conn-base-url")?.addEventListener(
    "input",
    () => showConnectionErrors({ baseUrl: "", token: currentTokenError() })
  );
  $2("#conn-token")?.addEventListener(
    "input",
    () => showConnectionErrors({ baseUrl: currentBaseUrlError(), token: "", llmKey: currentLlmKeyError() })
  );
  $2("#conn-llm-key")?.addEventListener(
    "input",
    () => showConnectionErrors({ baseUrl: currentBaseUrlError(), token: currentTokenError(), llmKey: "" })
  );
  function currentBaseUrlError() {
    const el = $2("#conn-base-url-err");
    return el && !el.classList.contains(HIDDEN) ? el.textContent : "";
  }
  function currentTokenError() {
    const el = $2("#conn-token-err");
    return el && !el.classList.contains(HIDDEN) ? el.textContent : "";
  }
  function currentLlmKeyError() {
    const el = $2("#conn-llm-key-err");
    return el && !el.classList.contains(HIDDEN) ? el.textContent : "";
  }
  $2("#detail-back")?.addEventListener("click", closeDetail);
  $2("#detail-star")?.addEventListener("click", () => {
    if (currentDetailId) wiring.onToggleFavorite?.(currentDetailId);
  });
  $2("#scrim")?.addEventListener("click", closeSheets);
  container.addEventListener("click", (e) => {
    ...
    if (folderToggle) {
      ...
      if (expandedSidebarFolders.has(folderPath)) expandedSidebarFolders.delete(folderPath);
      else expandedSidebarFolders.add(folderPath);
      renderSidebar(state);
      return;
    }
    ...
    if (open) {
      if ($2("#sheet-layer")?.classList.contains("open")) closeSheets();
  ...
  container.addEventListener("keydown", (e) => {
    if (e.key === "Escape") {
      ...
      else if ($2("#sheet-layer")?.classList.contains("open")) closeSheets();
