  ...
  function renderSidebar(state2) {
    ...
    if (nav) {
      ...
      nav.innerHTML = items.map((item) => {
        ...
        const rowAction = item.hasChildren ? ' data-folder-toggle="' + esc(item.value) + '" aria-expanded="' + (expanded ? "true" : "false") + '" aria-label="' + (expanded ? "\u6536\u8D77 " : "\u5C55\u5F00 ") + esc(item.label) + '"' : ' data-category="' + esc(item.value) + '" aria-pressed="' + (current === item.value ? "true" : "false") + '"';
        const leading = item.hasChildren ? '<svg class="sidebar-inline-chevron" viewBox="0 0 20 20" aria-hidden="true"><path d="m7 4 6 6-6 6"/></svg>' : '<span class="sidebar-inline-placeholder" aria-hidden="true"></span>';
        const openFolder = item.hasChildren ? '<button class="sidebar-open-folder" data-open-folder="' + esc(item.value) + '" data-category="' + esc(item.value) + '" aria-label="\u8FDB\u5165 ' + esc(item.label) + ' \u76EE\u5F55"><svg viewBox="0 0 20 20" aria-hidden="true"><path d="m7 4 6 6-6 6"/></svg></button>' : "";
        return '<div class="sidebar-tree-row" data-depth="' + item.depth + '" style="--tree-indent:' + Math.min(item.depth, 5) * 12 + 'px"><button class="sidebar-cat' + (current === item.value ? " active" : "") + '"' + rowAction + ">" + leading + '<svg class="sidebar-folder-icon" viewBox="0 0 24 24" aria-hidden="true"><path d="M3 7a2 2 0 0 1 2-2h4l2 2h8a2 2 0 0 1 2 2v8a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z"/></svg><span class="cat-name">' + esc(item.label) + '</span><span class="cat-count">' + item.count + "</span></button>" + openFolder + "</div>";
      }).join("");
    }
    if ($2("#sidebar-conn")) {
